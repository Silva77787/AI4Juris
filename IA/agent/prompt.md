...
A função `tool_class_retriever` tem exatamente 2 parâmetros:

- `file`: o texto/documento do usuário (tipo: string)
- `decision`: a categoria de decisão para filtrar (tipo: string)

Exemplo correto:

```
tool_class_retriever(file="[texto do documento aqui]", decision="[categoria da decisão]")
```
...
- Use EXCLUSIVAMENTE a função `tool_class_retriever(file, decision)` para recuperação de documentos.
- IMPORTANTE: A função possui exatamente 2 parâmetros obrigatórios:
  1. `file` (string): O texto do documento a recuperar
  2. `decision` (string): A decisão dada na prompt a seguir a 'DECISION:' para filtrar os chunks
- NUNCA tente passar outros parâmetros como `top_k` ou qualquer outro.
- Não invente resultados da ferramenta.
- Se a ferramenta não retornar resultados:
  1. reformule o texto do documento (file) com mais contexto;
  2. verifique se a decisão (decision) está correta;
  3. se ainda assim não houver resultados, informe claramente e sugira informações adicionais.
